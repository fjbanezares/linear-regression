...
Let's load some auxiliary functions:
```python
import numpy as np
...
def compute_cost(x, y, w, b):
    # number of rows, so number of samples in training set
    m = x.shape[0] 
    cost = 0
    # sum of all errors
    for i in range(m):
    ...
    return total_cost
```
Now we will use this to give the steps from the gradient descent
```python
def compute_gradient(x, y, w, b): 
    m = x.shape[0]        # Number of training examples
    dj_dw = 0
    dj_db = 0
    # We here iterate in all samples calculating the gradient
    for i in range(m):  
        f_wb = w * x[i] + b 
...
visualization of gradient and cost:
```python
...
Nos the steps become baby steps when approaching the minimum
```python
...
Big learning rate makes divergence
```python
# initialize parameters
w_init = 0
b_init = 0
...
```python
def plt_divergence_small(p_hist, J_hist, x_train,y_train):
    x=np.zeros(len(p_hist))
    y=np.zeros(len(p_hist))
    v=np.zeros(len(p_hist))
    for i in range(len(p_hist)):
    ...
    # Second Subplot
    #===============
    tmp_b,tmp_w = np.meshgrid(np.arange(-350, 350, 2),np.arange(-700, 700, 2))
    z=np.zeros_like(tmp_b)
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j] )
...
```python
plt_divergence_small(p_hist, J_hist,x_train, y_train)
plt.show()
```
